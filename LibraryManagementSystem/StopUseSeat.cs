...
namespace LibraryManagementSystem
{
    ...
    public partial class StopUseSeat : Form
    {
        ...
        private void StopUseSeat_Load(object sender, EventArgs e)
        {
            ...
            MySqlDataAdapter da = new MySqlDataAdapter("select startTime from seattbl where UserID='"+NameLabel.Text.ToString()+"' AND seatNumber="+int.Parse(seatNumlabel.Text.ToString())+" AND endTime IS NULL", SUSconnection);
            da.Fill(ds);
        ...
        {
            //select* from seattbl where UserID = 'gou1226' AND seatNumber = 25 AND endTime IS NULL;

            string insertQuery = "update seattbl set endTime='"+DateTime.Now.ToString()+"' where UserID='"+NameLabel.Text.ToString()+"' AND seatNumber="
